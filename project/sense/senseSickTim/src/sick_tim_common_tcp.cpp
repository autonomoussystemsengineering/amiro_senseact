...
SickTimCommonTcp::SickTimCommonTcp(const std::string &hostname, const std::string &port, int &timelimit, AbstractParser* parser, std::string rsboutscope)
:
    SickTimCommon(parser, rsboutscope),
    socket_(io_service_),
...
int SickTimCommonTcp::init_device()
{
    ...
    {
        printf("Could not resolve host: ... (%d)(%s)", e.code().value(), e.code().message().c_str());
        return ExitError;
    }
    ...
    for ( ; iterator != tcp::resolver::iterator(); ++iterator)
    {
        ...
        printf("Waiting %i seconds for device to connect.", timelimit_);
        deadline_.expires_from_now(boost::posix_time::seconds(timelimit_));

        ec = boost::asio::error::would_block;
        printf("Attempting to connect to %s", repr.c_str());
        ...
        {
            success = true;
            printf("Succesfully connected to %s", repr.c_str());
            break;
        }
        printf("Failed to connect to %s", repr.c_str());
    }

    // Check if connecting succeeded
    if (!success)
    {
        printf("Could not connect to host %s:%s", hostname_.c_str(), port_.c_str());
        return ExitError;
    }
...
int SickTimCommonTcp::close_device()
{
    if (socket_.is_open())
    {
        ...
        {
            printf("An error occured during closing of the connection: %d:%s", e.code().value(), e.code().message().c_str());
        }
    }
    return 0;
}
...
int SickTimCommonTcp::readWithTimeout(size_t timeout_ms, char *buffer, int buffer_size, int *bytes_read, bool *exception_occured)
{
    ...
    if (buffer != 0)
    {
        ...
        {
            printf("Dropping %zu bytes to avoid buffer overflow", bytes_transfered_ - to_read);
...
int SickTimCommonTcp::sendSOPASCommand(const char* request, std::vector<unsigned char> * reply)
{
    ...
    {
        printf("sendSOPASCommand: no full reply available for read after 1s");
        return ExitError;
    }
...
int SickTimCommonTcp::get_datagram(unsigned char* receiveBuffer, int bufferSize, int* actual_length)
{
    ...
    {
        printf("get_datagram: no data available for read after %zu ms", timeout);
