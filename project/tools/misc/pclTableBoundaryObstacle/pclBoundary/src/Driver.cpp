...
pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr Driver::startDriver(const pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr &cloud)
{
    ...
    {
      pcl::PointXYZRGBA p = cloud->at(inliers->indices[i]);
      p.rgb = green ;
      plane->push_back(p);
    }
    */
...
pcl::PointCloud<pcl::PointXYZRGBA>::Ptr Driver::pointCloudTo2DScanPlane(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloudOnPlane)
{
  std::vector<float> scan(360 ,0.0f);//large initial value , field of view is 60 so taking 120 values
