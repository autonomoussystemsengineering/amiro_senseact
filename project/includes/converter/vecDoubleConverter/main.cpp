...
AnnotatedData CLASS_NAME_::deserialize(const string& wireSchema,
    ...
    std::vector<TYPE_PAYLOAD_>* vector = new std::vector<TYPE_PAYLOAD_>( (TYPE_PAYLOAD_*) &*(wire.begin() + SIZE_PAYLOAD_), (TYPE_PAYLOAD_*) &*(wire.begin() + SIZE_UINT_ + SIZE_PAYLOAD_ * uiVecSize) );
