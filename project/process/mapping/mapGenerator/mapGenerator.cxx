...
// Generate a Mapupdate for a detected edge
MapUpdate MapGenerator::createEdgePoseUpdate(cv::Point3f edgePose) {
...
//	int edgeRadius = (int) (cv::norm(diff) / cellSize);
	...
	// calculate edge coordinates
	cv::Point2i mid(mapUpdateSize.width / 2, mapUpdateSize.height / 2);
	cv::Point2f p0(1.5 * robotRadius * cos(edgePose.z), 1.5 * robotRadius * sin(edgePose.z));
	cv::Point2i p1((int) ((p0.x + robotRadius * cos(edgePose.z + M_PI / 2)) / cellSize),
			(int) ((p0.y + robotRadius * sin(edgePose.z  + M_PI / 2)) / cellSize));
	cv::Point2i p2((int) ((p0.x + robotRadius * cos(edgePose.z - M_PI / 2)) / cellSize),
			(int) ((p0.y + robotRadius * sin(edgePose.z - M_PI / 2)) / cellSize));
