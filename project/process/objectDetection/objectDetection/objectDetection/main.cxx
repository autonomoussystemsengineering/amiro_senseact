...
void threadedHomography(std::vector<Point2f> obj, std::vector<Point2f> scene) {
  threadedMat = findHomography(obj, scene, CV_RANSAC);
}
...
/*        Mat H;
        boost::thread findingThread (threadedHomography, obj, scene);
        findingThread.join();
        threadedMat.copyTo(H);
*/
      ...
      bool detected = detectedFeatures && detectedDist && detectedLarge && detectedDia && detectedPos;
      if (detected) {
        detectObjId = objId;
      }
      ...
      if (!commandQueue->empty()) {
        ...
        // check for storing command
          ...
          if (!loadingDirectly) {
            ...
            INFO_MSG("Objects already loaded in the beginning.");
          }
