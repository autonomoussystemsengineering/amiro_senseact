...
Mat threadedMat[maxObjectSites];
...
void threadedHomography(std::vector<Point2f> obj, std::vector<Point2f> scene, int siteId) {
  Mat result = findHomography(obj, scene, CV_RANSAC);
  result.copyTo(threadedMat[siteId]);
}
...
int searchObjectsInScene(Mat img_scene) {
  ...
  for (int objIdFor=0; objIdFor<objectCount; objIdFor++) {
    ...
    bool detectedFeatures[maxObjectSites];
    Mat img_object[maxObjectSites];
    Mat img_matches[maxObjectSites];

    for (int siteId=0; siteId<maxObjectSites; siteId++) {

      // load object keypoints and descriptor
      objectPics[objId][siteId].copyTo(img_object[siteId]);
      ...
      if (debugging) {
        drawMatches( img_object[siteId], keypoints_object, img_scene, keypoints_scene,
               good_matches, img_matches[siteId], Scalar::all(-1), Scalar::all(-1),
      ...
      detectedFeatures[siteId] = obj.size() >= 4 && scene.size() >= 4;
      if (detectedFeatures[siteId]) {
        threads[siteId] = boost::thread(threadedHomography, obj, scene, siteId);
      }
    }
    ...
    for(int siteId=0; siteId<maxObjectSites; siteId++) {
      ...
      if (detectedFeatures[siteId]) {
        ...
        tBeforeHomography = clock();
        Mat H;
        threadedMat[siteId].copyTo(H);
        tAfterHomography = clock();
        ...
        if (debugging) {
          ...
          line( img_matches[siteId], scene_corners[0] + Point2f( img_object[siteId].cols, 0), scene_corners[1] + Point2f( img_object[siteId].cols, 0), color, 4 );
          line( img_matches[siteId], scene_corners[1] + Point2f( img_object[siteId].cols, 0), scene_corners[2] + Point2f( img_object[siteId].cols, 0), color, 4 );
          line( img_matches[siteId], scene_corners[2] + Point2f( img_object[siteId].cols, 0), scene_corners[3] + Point2f( img_object[siteId].cols, 0), color, 4 );
          line( img_matches[siteId], scene_corners[3] + Point2f( img_object[siteId].cols, 0), scene_corners[0] + Point2f( img_object[siteId].cols, 0), color, 4 );
        }
      } else {
        tObjEnd = clock();
      }

      bool detected = detectedFeatures[siteId] && detectedDist && detectedLarge && detectedDia && detectedPos;
      if (detected) {
        detectObjId = objId;
      }
      ...
      if (debugging) {
        printf(" - %i us (%i, %i)", int(tObjEnd-tObjStart), int(tAfterMatch-tBeforeMatch), int(tAfterHomography-tBeforeHomography));
      }
      printf("\n");

      if (debugging) {
        string filename = siftDirectory + savingDir + to_string(comparisonCount) + "_" + to_string(objId) + "_" + to_string(siteId) + fileend;
        bool ok = imwrite(filename, img_matches[siteId]);
        if (!ok) {
